...
export default function ResizePage() {
  ...
  const [resizeMode, setResizeMode] = useState<"none" | "percent" | "exact">("percent");
  ...
  const optionsFor = useCallback(
    (id: string): ConvertOptions => {
      return {
        ...
        quality: 100,
        resizeMode: applyToAll ? resizeMode : "none",
        resizePercent,
        exactWidth,
        exactHeight,
        maintainAspect,
    ...
    [applyToAll, resizeMode, resizePercent, exactWidth, exactHeight, maintainAspect],
  );

  const globalIsNoOp = useMemo(
    () => resizeMode === "none" || (resizeMode === "percent" && resizePercent === 100),
    [resizeMode, resizePercent],
  );
    ...
    <>
      {images.length === 0 ? (
        ...
        <>
          <div className="sticky top-16 z-30 -mx-4 mb-5 border-b border-border bg-paper/95 px-4 py-3 backdrop-blur-md sm:-mx-6 sm:px-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="absolute left-0 top-full z-50 mt-2 w-72 rounded-xl border border-border bg-white p-3 shadow-lg dark:bg-black">
                      ...
                      <div className="pr-7">
                        <SegmentedControl
                          ...
                          value={resizeMode === "none" ? "percent" : resizeMode}
                      ...
                      {resizeMode === "exact" && (
                        <div className="mt-3 flex flex-wrap items-end gap-3">
                          <label className="block">
                            <span className="mb-1 block text-xs text-ink-faint">Width</span>
                            <input
                              type="number"
                              value={exactWidth}
                              onChange={(e) => setExactWidth(Number(e.target.value))}
                          ...
                          {!maintainAspect && (
                            <label className="block">
                              <span className="mb-1 block text-xs text-ink-faint">Height</span>
                              <input
                                type="number"
                                value={exactHeight}
                                onChange={(e) => setExactHeight(Number(e.target.value))}
                                className="w-24 rounded-md border border-border bg-paper px-2 py-1.5 text-sm text-ink"
                              />
                            </label>
                          )}
              ...
              <div className="flex items-center gap-2">
                <label
                  title="Apply the same resize settings to all images. Turn off to customize each image separately."
          ...
          <button
            type="button"
            onClick={() => document.getElementById("add-more-input-resize")?.click()}
