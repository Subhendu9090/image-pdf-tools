...
/**
 * NOTE: `lib/image-convert.ts` (as used by the original page) only models
 * format/quality/resize in ConvertOptions — there's no sharpen/brighten
 * field. The sliders below are fully wired to local state, but the
 * `enhance` key on ConvertOptions is not a real option yet — extend
 * ConvertOptions + convertImage with that field when ready. Until then,
 * processing here is effectively a no-op pass-through.
 */
  ...
  const optionsFor = useCallback(
    (id: string): ConvertOptions => {
      return {
        ...
        exactHeight: 0,
        maintainAspect: true,
        // @ts-expect-error -- enhance is not yet defined on ConvertOptions
        enhance: applyToAll ? { sharpen, brighten } : undefined,
      };
    },
    [applyToAll, sharpen, brighten],
  );

  const globalIsNoOp = useMemo(() => sharpen === 0 && brighten === 0, [sharpen, brighten]);
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative">
                  ...
                  {enhanceOpen && (
                    <div className="absolute left-0 top-full z-50 mt-2 w-72 rounded-xl border bg-white p-3 shadow-lg dark:bg-black">
                      <button
                        type="button"
                ...
                >
                  ...
                  {!applyToAll && (
                    <p className="text-xs text-ink-faint">
                      Per-image enhance overrides aren&apos;t supported yet — using global settings.
                    </p>
                  )}
